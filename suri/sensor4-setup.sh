...
apt-add-repository -y ppa:oisf/suricata-stable
apt-get update
...
cp /vagrant/local.rules /etc/suricata/rules

# Generate /etc/snort/sid-msg.map
/usr/share/oinkmaster/create-sidmap.pl /etc/suricata/rules > /etc/suricata/sid-msg.map
touch /etc/snort/gen-msg.map

# Run barnyard2
( barnyard2 -c /vagrant/barnyard2.conf -v -f unified2.alert -d /var/log/nsm -i eth1 -h sensor4 2>&1 | tee -a /var/log/barnyard2.out | logger -t barnyard -i --server 192.168.33.9 -P 8514 -u /dev/null ) &
